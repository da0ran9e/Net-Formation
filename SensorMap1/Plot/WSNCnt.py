...
def parse_wsn_map(file_path):
    ...
    connected_nodes = set()  # Track nodes that already established a connection
    ...
    for line in lines:
        ...
        if match:
            ...
            # Ensure each node connects only to its first connection
            if from_node not in connected_nodes:
                ...
                print(f"Node {from_node} already connected. Skipping connection to node {to_node}.")
        else:
...
wsn_file_path = 'WSNMap3.txt'  # Replace with your actual file path
...
plt.savefig("sensor_map_con_03.png")
