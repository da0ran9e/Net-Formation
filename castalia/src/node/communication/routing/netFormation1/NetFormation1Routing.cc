...
#include "node/communication/routing/netFormation1Routing/NetFormation1Routing.h"

Define_Module(NetFormation1Routing);

void NetFormation1Routing::startup()
{
	// Set timeout
...
void NetFormation1Routing::sendTopologySetupPacket()
{
	NetFormation1RoutingPacket *setupPkt =
	    new NetFormation1RoutingPacket("Multipath rings routing setup packet", NETWORK_LAYER_PACKET);
	setupPkt->setNetFormation1RoutingPacketKind(MPRINGS_TOPOLOGY_SETUP_PACKET);
...
void NetFormation1Routing::sendControlMessage(NetFormation1RoutingControlDef kind)
{
	NetFormation1RoutingControlMessage *ctrlMsg =
	    new NetFormation1RoutingControlMessage("Multipath routing control message",NETWORK_CONTROL_MESSAGE);
	ctrlMsg->setNetFormation1RoutingControlMessageKind(kind);
	ctrlMsg->setLevel(currentLevel);
	ctrlMsg->setSinkID(currentSinkID);
	toApplicationLayer(ctrlMsg);
}

void NetFormation1Routing::timerFiredCallback(int index)
{
...
void NetFormation1Routing::processBufferedPacket()
{
...
void NetFormation1Routing::fromApplicationLayer(cPacket * pkt, const char *destination)
{
	string dst(destination);

	NetFormation1RoutingPacket *netPacket =
	    new NetFormation1RoutingPacket("Multipath rings routing data packet", NETWORK_LAYER_PACKET);
	netPacket->setNetFormation1RoutingPacketKind(MPRINGS_DATA_PACKET);
...
void NetFormation1Routing::fromMacLayer(cPacket * pkt, int macAddress, double rssi, double lqi)
{
	NetFormation1RoutingPacket *netPacket = dynamic_cast <NetFormation1RoutingPacket*>(pkt);
	if (!netPacket)
		return;

	switch (netPacket->getNetFormation1RoutingPacketKind()) {
			...
			if (dst.compare(BROADCAST_NETWORK_ADDRESS) == 0 ||
				...
				if (senderLevel == currentLevel + 1) {
					if (self == sinkID) {
						...
						NetFormation1RoutingPacket *dupPacket = netPacket->dup();
